...
with st.form("form"):
    ...
    st.write('<style>div.row-widget.stRadio > div{flex-direction:row;}</style>', unsafe_allow_html=True)
    style = st.radio("style", ["null", "cute", "Expressive", "Scenic"])
    st.write('<style>div.row-widget.stRadio > div{flex-direction:row;}</style>', unsafe_allow_html=True)
    Quality = st.radio("Quality", ["1", ".25", ".5"])
    st.write('<style>div.row-widget.stRadio > div{flex-direction:row;}</style>', unsafe_allow_html=True)
    Stylize = st.slider("select Stylize", 0, 1000, [100])
    
    Prompt = f"an image of Niji colors: {Prompt}, Parameter : [Aspect:{Aspect}, style:{style}, Quality:{Quality}, Stylize:{Stylize}]"
    submit = st.form_submit_button("Submit")
    
    
if submit and Prompt:
    with st.spinner("Waiting for Niji..."):
